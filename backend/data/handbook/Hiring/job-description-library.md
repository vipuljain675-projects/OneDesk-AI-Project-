---
title: Job Description Library Overview
description: "Job descriptions are organized by function at GitLab and we use them to inform candidates of roles and current team members to evaluate their performance."
---

- Job descriptions are [organized by function in directories in the `content-sites/handbook` repository](https://gitlab.com/gitlab-com/content-sites/handbook/-/tree/main/content/job-description-library/).
- View job descriptions in the [Job Description Library](/job-description-library/).
- For any questions related to job descriptions, please reach out to your Manager.

### Things to Keep in Mind

1. Before you begin creating a new job description you should check the directory to see if there is an existing job description or a role close to the one you are working to create. Job families should encompass a team, a department, or perhaps a division.
    - Sometimes you should just add a level, a specialty or a segment and not create a new job description.
1. We don't include location requirements for most roles (EMEA, Americas, APAC) in the job description because these can change over time and job descriptions are constant.
...
1. Team Lead vs people Manager vs function Manager: you should be aware of how GitLab defines these terms. Depending on the needs of the organization, a Team Lead may or may not directly manage people. There are 2 instances where a Team Lead role can be used:
    ...
    - Manager job descriptions are those responsible for directly managing other GitLab team-members. They hire, promote, and terminate team members, and performance management is one of their key functions. Nomenclature to represent someone is a manager of people can be clarified with "Manager, Benchmark" (e.g., Manager, Corporate Events). The same holds true for director (e.g. Director, Corporate Events).
...
Anyone can create or edit a job description, but generally it is the responsibility of the person who will manage the position or job description to kick off the approval process. They may enlist help from others as necessary. After a merge request is created for an existing or new job description, **the merge request must follow the approval flows below**:

1. **Your manager**: Your direct manager is responsible for clarifying the scope of responsibilities and level of roles, ensuring clear differentiation between levels, and checking responsibilities and requirements and ensuring the job description follows the template logic and **has all of the required areas**. Mangers should validate business need for the role with their direct manager. Any review after this should be quick so the manager is the gatekeeper.
1. **Talent Management**: Talent Management will review for job level expectations consistency with other similarly leveled roles across the organization, alignment to our [GitLab Job Frameworks](https://docs.google.com/spreadsheets/d/1FX4NBwF099uMBm7mGBtf1orIJZuHEjtiEa3jSbg9jJs/edit?gid=0#gid=0), and overall structure, then notify your People Business Partner (PBP).
...
1. **Total Rewards**: The Total Rewards Team, `@gl-total-rewards`, is the final approver for job descriptions on the merge request ensuring.:

   - The level is common in the market based on survey data.
   - All components of the job description are populated.
...
## Templates for New Job Description

There are two templates on this page. The first template is appropriate when the job description has 1 or 2 job titles and more levels (i.e. junior, intermediate, senior, manager, director) or specialties; [for example](/job-description-library/engineering/backend-engineer). There can be many levels for each of the titles. The second template is appropriate when there are many different job titles within the same family; [for example](/job-description-library/people-group/total-rewards).
...
```markdown
---
title: {add text of job description name}
---

{add overview; a brief description of the job description}
...
The {add the position title} reports to the {add the reporting position and link to the job description or heading (if on the same page)}.

#### {add Level} Job Level

The {add the role name} is outlined in the [Job Levels](https://docs.google.com/spreadsheets/d/1kcDb-A2uwchPtTNSJON65BdqS9P0KQmNz0fbNMZMt_M/edit?gid=819074618#gid=819074618) resource.
...
- {add a bulleted list of responsibilities that apply for all levels of this job description}

#### {add Level} Requirements

- {add a bulleted list of requirements that apply for all levels of this job description}
...
- List any overarching responsibilities or requirements that are applicable for the entire job description before listing the levels.
...
- [Segment](/handbook/sales/field-operations/gtm-resources/): A Segment is something you will see with Sales based Job Descriptions (Example, Enterprise, Mid-market, SMB, Public Sector). If there is no segment for the job description remove the section.
- If performance indicators are the same for all the levels on the job description do not list them under the levels section, rather list them at the end of the entire job description. Remove either the performance indicator section in the level section or at the end of the template.
- Career Ladder: The next job description. All levels should be on one job description, except when the next job description manages over multiple job descriptions. For [example](/job-description-library/people-group/people-operations#career-ladder). Some teams added a Career Path: **Moving to and moving from*- which describes possible current and future roles which move to the job description and which the job description might move to. Here is [an example](/job-description-library/product/product-manager#moving-from-product-management).
- The **Compensation** and **About GitLab** sections will auto-populate because of the job description formatting.
...
{add an overview; a brief description of the job description}

## Role

{add brief description of the role} The {add the position title} reports to the {add the reporting position and link to the job description or heading (if on the same page).}.

### Job Level

The {add role name} is outlined in the [Job Levels](https://docs.google.com/spreadsheets/d/1kcDb-A2uwchPtTNSJON65BdqS9P0KQmNz0fbNMZMt_M/edit?gid=819074618#gid=819074618) resource.
...
## Creating a New Job Description from Scratch
...
1. Go to the [job-description-library](https://gitlab.com/gitlab-com/content-sites/handbook/-/tree/main/content/job-description-library/) directory
1. Select the department where you will be making the new job description (i.e. Marketing, Sales, etc)
...
1. Copy the [Template for New Job Description](#templates-for-new-job-description) and insert it into the body of the file
...
In cases where a job description is quite large you may decide to create an overview page of the job description. This helps when we are actively sourcing for external candidates on a role.

{{< youtube "eECNGB-JkZE" >}}

- Keep in mind that the overview page needs to be updated when you add a new level or a new role to a job description.
- The opening statement about the job description should be copied from the job description page to the overview page. Because not all job descriptions have overview pages you should not remove it from one or the other.
- The overview page should follow the typical naming convention by adding a sub folder `overview`

### Format

For every overview page we created, the page has the URL `/job-description-library/department/title/overview/` and should be
created starting from the template. There's only a few additions you should do with the template.

### New Job Description Overview creation

1. Create the folder in `/job-description-library/[department]/[name-of-job-family]/overview`, being sure to use only lower case in naming your directory if it doesn't already exist, and add it to the correct department subdirectory.
...
1. Update all the places that need updates:
   ...
   - replace {Job Description introduction} with a paragraph giving more information about the job itself.
   - if there's a video for this job description, switch out the youtube `ID_FOR_VIDEO` with the relevant ID
   - if there's no video, remove the entire snippet that adds the video
   - for every role within the job description, you need a `card`.

### Template for New Job Description Overview
...
This video will walk you through how to change a Job Description URL as well as set up a redirect for the old URL.

{{< youtube "MRzgOzlbsPQ" >}}

## Failed Pipeline

We have a CI job that checks job descriptions for hard requirements:

- Requirements
  - every role must have `Ability to use GitLab` as a bullet point.
- Responsibilities
- Performance Indicators
- [Job Levels](https://docs.google.com/spreadsheets/d/1kcDb-A2uwchPtTNSJON65BdqS9P0KQmNz0fbNMZMt_M/edit?gid=819074618#gid=819074618)
- Hiring Process
- Inclusive Language Check

### Inclusive Language Check

We use an open source [Ruby gem](https://gitlab.com/lienvdsteen/linter) to perform a inclusive language check on all job descriptions.
...
- High usage of masculine-coded language: this means the job description is using more masculine-coded words than feminine-coded words.
- Usage of any misused words as this could excluded diverse groups.
- High usage of fixed-coded language: this means the job description is using more fixed-coded words than growth-coded.
- Usage of gendered pronouns: we want to be inclusive to all.
  ...
  - If the pipeline does fail, the first recommendation is to read the error. It will say what is wrong with the text. For example:
   ...
   ```text
   1. ["ATTENTION: In /builds/gitlab-com/www-gitlab-com/sites/uncategorized/source/job-description-library/marketing/reference-program-manager/index.html.md you're using masculine gender-coded language", "Masculine coded words used: analyst, analytical, decision, driven, leader"]
   2. ["ATTENTION: In /builds/gitlab-com/www-gitlab-com/sites/uncategorized/source/job-description-library/marketing/production-designer/index.html.md you're using fixed-coded language", "Fixed coded words used: established"]
   ```

   In this case, there are two job descriptions that failed and each for a different reason. You can do two things now:
      - fix the text to be more inclusive, commit your changes and the pipeline will run again
      - if you disagree or you feel like the pipeline found a false positive, you can add the file to [this list](https://gitlab.com/gitlab-com/www-gitlab-com/-/blob/master/data/inclusiveness_check.yml). The files in this list are ignored by the inclusiveness check. If you request your job description to be added to the skip level you will need to paste a screenshot of the inclusiveness check in the MR as this will be asked for prior to merging.

## Why Job Descriptions Have Performance Indicators

{{< youtube "9EJkgBRUSDA" >}}

- All job descriptions should have performance indicators (PIs).
- Performance indicators should tell you whether or not you're doing your job well or correctly. For example, our [Vice President, Investor Relations](/job-description-library/finance/investor-relations#vice-president-investor-relations) role has performance indicators that compare how we're being described by analysts to how we describe ourselves.
...
- Company, functional, or department KPIs are too generic and, thus, not useful as job description performance indicators, only a sub-set of performance indicators are KPIs.
...
- Job Descriptions should have three to five PIs.
- Performance Indicators should be linked to one or more performance indicator definitions for which this job description will be the [DRI](/handbook/people-group/directly-responsible-individuals/).
